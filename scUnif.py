...
def gem2csv(dirname, gem, prefix=""):
    ...
    if gem.hasSC:
        mtx2csv(prefix + 'exp_S.csv', gem.suff_stats['exp_S'].transpose())
...
def load_from_file(filename, dtype=float, delimiter=","):
    if filename is None:
        ...
        if filename.rsplit(".", 1)[1] == "mtx":
            return sio.mmread(filename).toarray().transpose().astype(np.float32)  # transposition for gene-bc matrix
        else:
...
if __name__ == "__main__":
    ...
    myGEM = LogitNormalGEM(
                  BKexpr=BKexpr, SCexpr=SCexpr, G=G, K=K,
                  iMarkers=iMarkers,
    ...
    sio.mmwrite("{}/{}_impute.mtx".format(args.output_directory, args.output_prefix), coo_matrix(myGEM.suff_stats['exp_S']).transpose())
